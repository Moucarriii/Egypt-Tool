...
exrg_future = [
    st.sidebar.number_input(f"ER Growth for {dt:%b %Y}", value=0.0,
...
gi_future = [
    st.sidebar.number_input(f"Global Inflation for {dt:%b %Y}", value=0.0,
...
# 5. Chart type toggle
# --------------------------------------------------------------------------
view = st.radio("View:", ("Yearly average", "Monthly detail"), horizontal=True)
...
if view == "Yearly average":
    ...
    # Filter by last historical year using boolean indexing
    hist_monthly = df_hist[df_hist['Year'].dt.year == last_year][['Year','Egypt Inflation']].rename(columns={'Egypt Inflation':'Inflation'})
    ...
    st.altair_chart((line + pts).properties(width=700, height=350),
                    use_container_width=True)
